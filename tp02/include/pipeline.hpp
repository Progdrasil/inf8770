...
int code(fs::path path, bool subsampling, uint quantifLevel, std::vector<cv::Mat_<int>> * quantif, std::vector<cv::Size> * oSize, std::vector<uint> * lineSizes);
int decode(std::vector<cv::Mat_<int>> quantif, uint quantifLevel, std::vector<cv::Size> oSize, std::vector<uint> lineSizes);

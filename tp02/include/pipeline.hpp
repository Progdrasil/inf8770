...
int code(fs::path path, bool subsampling, int quantifLevel, uint8_t **huffmanData, std::vector<cv::Size> &oSize, std::vector<uint> &lineSizes, std::vector<int> &compressionData);
int decode(fs::path path, uint8_t **huffmanData, int quantifLevel, std::vector<cv::Size> &oSize, std::vector<uint> &lineSizes, std::vector<int> &compressionData);
